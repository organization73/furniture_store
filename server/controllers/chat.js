...
exports.createChatRoom = async (req, res, next) => {
  ...
  try {
    ...
    const chatRoom = await ChatRoom.findOne({users:{$all:[primaryUser._id, secondaryUser._id]}});
    if (chatRoom) {
      throwError('Chat room already exists', 400, 'chatRoom');``
      // return res.status(200).json({message: 'Chat room already exists', chatRoom});
    }
    //create chat room
    const newChatRoom = new ChatRoom({users: [primaryUser._id, secondaryUser._id]});
    if (req.admin){
    ...
    res.status(201).json({message: 'Chat room created', chatRoom: newChatRoom});
  }catch(err){
    if (!err.statusCode) {
      err.statusCode = 500;
    }
